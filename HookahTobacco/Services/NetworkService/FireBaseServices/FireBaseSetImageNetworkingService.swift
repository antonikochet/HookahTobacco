...
class FireBaseSetImageNetworkingService: SetImageNetworkingServiceProtocol {
    ...
    func setImage(
        from oldImage: ImageNetworkingDataProtocol,
        to newURL: URL,
        for newImage: ImageNetworkingDataProtocol,
        completion: @escaping Completion
    ) {
        var deleteError: NetworkError?
        ...
        newRef.putFile(from: newURL) { [weak self] _, error in
            guard let self = self else { return }
            if error != nil { completion(self.handlerErrors.handlerError(error!)) } else { completion(deleteError) }
        }
    }
